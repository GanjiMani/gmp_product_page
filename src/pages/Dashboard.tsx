...
import { Building2, FileText, TrendingUp, Activity, Globe, AlertTriangle } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [, setLoadingProgramAreas] = useState(true);
  const [, setLoadingClassifications] = useState(true);
  const [, setLoadingCountrywise] = useState(true);
  const [, setLoadingTrend] = useState(false);
  ...
  useEffect(() => {
    const fetchProgramAreas = async () => {
      try {
        ...
        const transformedCounts: ProgramAreaCounts = {};
  ...
  useEffect(() => {
    const fetchObservations = async () => {
      try {
        ...
        const response: any = await fetchTrend483Observations(
        ...
        let observationsData: ObservationRecord[] = [];
              ...
              <ResponsiveContainer width="100%" height={400}>
                <BarChart data={countryData.slice(0, 10)} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                  ...
                  <Bar dataKey="count" radius={[8, 8, 0, 0]} strokeWidth={2}>
                    {countryData.slice(0, 10).map((_, index) => (
                      <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} stroke={COLORS[index % COLORS.length]} strokeWidth={2} />
                    ))}
                  </Bar>
                </BarChart>
