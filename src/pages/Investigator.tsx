...
import { fetchTotalInvestigators, fetchInvestigators, Investigator as InvestigatorType } from '../services/api';

const Investigator = () => {
  const [total, setTotal] = useState<number | null>(null);
  const [topInvestigators, setTopInvestigators] = useState<InvestigatorType[]>([]);
  const [searchName, setSearchName] = useState<string>('');
  const [searchResults, setSearchResults] = useState<InvestigatorType[]>([]);
  ...
  useEffect(() => {
    const loadTotal = async () => {
      try {
        ...
        // Assuming data structure based on original usage: data.value
        setTotal(data.value);
      } catch (err) {
        console.error(err);
      ...
      <table className="min-w-full bg-white shadow rounded-lg overflow-hidden">
        ...
        <tbody>
          ...
          {displayList.map((inv, index) => (
            <tr key={`${inv.investigator_name || inv.name}-${index}`} className="border-b hover:bg-gray-50">
